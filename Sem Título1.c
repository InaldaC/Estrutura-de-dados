...
//	recuperaDados();
	...
	void meuMenu(){
		...
		printf("\n\t\t 4. Listar os jogadores existentes \n\t\t 5. Elimanar o 3 jogador com mais golos.\n");
		int op;
		scanf("%d", &op);
		if(op==1){
			adicionarEquipe();
...
void registraJogadores(){
 ...
 printf("\n\t\t\t\t\t------ESTA E A SECCAO CADASTRO--------");
    ...
    for(i=0;i<num; i++){
        ...
        FILE *arquivo = fopen("Player.dat","ab");
        //j = joga[i];
...
int nrDeElementos(){
	FILE* arquivo = fopen("Player.dat","rb");
	Jogador j;
	int i=0;
	if(arquivo==NULL){
	...
	for(k=0;k<aux;k++){
		...
		//jog[aux].erase(jog[aux.begin()+2]);
		fclose(arquivo);
}
	File*arquivo=fopen("Player.dat", "w");
	if(arquivo=)	    
for(k=2;k<aux-2; k++){
	jg[k]=jg[k+1];
	--aux;
